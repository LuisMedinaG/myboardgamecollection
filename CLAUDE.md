...
- **Existing custom CSS** — during migration, old CSS lives in `static/styles/`. As each page converts to Tailwind, remove its `@import` from `static/input.css` and delete the file. Remove `pico.min.css` last.
...
Go 1.25 · stdlib HTTP server · HTMX (no JS framework) · **Tailwind CSS v4** (migrating from Pico CSS) · SQLite (`modernc.org/sqlite`) · Docker + Fly.io

## CSS Architecture (Tailwind v4)

- **Source file**: `static/input.css` — Tailwind config + `@import` of any remaining legacy CSS
...
- **Legacy CSS**: `static/styles/` — being removed page-by-page; only keep what isn't yet migrated
...
static/
  ...
  styles/              # Legacy CSS files (removed during Tailwind migration)
```
